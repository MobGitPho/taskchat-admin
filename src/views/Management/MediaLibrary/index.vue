<script setup lang="ts">
  ...
  const nbTrashedMediaFiles = ref(0)
  ...
  const init = async (url: string | null = null, loadAllData = false) => {
    ...
    if (loadAllData) {
      ...
      const task8 = await request({
        url: `/count/media-files/trashed`,
      })

      if (task8.success && task8.result) {
        nbTrashedMediaFiles.value = task8.result.data
      }
    }
  ...
  const deleteItem = async (item: MediaFileModel) => {
    dialog.warning({
      ...
      onPositiveClick: async () => {
        let task
        if (filterType.value == TRASH_ID) {
          task = await request({
            method: 'delete',
            url: `/media-files/force/${item.id}`,
          })
        } else {
          task = await request({
            method: 'delete',
  ...
  const restoreItem = async (item: MediaFileModel) => {
    dialog.warning({
      title: t('confirm'),
      content: t('confirmRestoreMediaFile'),
      positiveText: t('yes'),
      negativeText: t('no'),
      onPositiveClick: async () => {
        const task = await request({
          method: 'put',
          url: `/media-files/restore/${item.id}`,
        })
  ...
  const emptyTrash = async () => {
    dialog.warning({
      title: t('confirm'),
      content: t('confirmEmptyTrash'),
      positiveText: t('yes'),
      negativeText: t('no'),
      onPositiveClick: async () => {
        const task = await request({
          method: 'delete',
          url: `/media-files/force/all`,
        })
  ...
  const restoreSelection = async () => {
    if (checkedItemsNumber.value) {
      dialog.warning({
        title: t('confirm'),
        content: t('restoreSelectionWarningMessage', [
          checkedItemsNumber.value,
        ]),
        positiveText: t('yes'),
        negativeText: t('no'),
        onPositiveClick: async () => {
  ...
  <PageLayout>
    <template #header-extra>
      <section v-if="checkedItemsNumber > 0" class="flex items-center gap-3">
        ...
        <n-button
          v-if="filterType == TRASH_ID"
          type="success"
          size="small"
          @click="restoreSelection">
          <template #icon>
            <n-icon>
              <Icon icon="mdi:delete-restore" />
            </n-icon>
          </template>
          {{ t('restore') }}
        </n-button>
      ...
      <section v-else class="flex items-center gap-3">
        ...
        <n-button
          ...
          @click="
            () => {
              router.push(Routes.MEDIA_LIBRARY.PATH + '/add')
            }
          ">
          <template #icon>
            <n-icon>
    ...
    <n-card size="small" class="mb-5">
      <div class="flex justify-between items-center">
        ...
        <n-radio-group
          ...
          <n-radio-button
            v-for="mediaFileType in mediaFileTypes"
          ...
          <n-radio-button :value="TRASH_ID">
            {{ t('trash') }}&nbsp;
            <span v-if="nbTrashedMediaFiles">({{ nbTrashedMediaFiles }})</span>
          </n-radio-button>
        </n-radio-group>
      </div>
    </n-card>
    <n-card embedded class="py-2">
      <section v-if="mediaFiles.length && !isLoading">
        <n-image-group show-toolbar-tooltip>
          <n-grid x-gap="12" y-gap="12" cols="1 400:2 600:3 900:4">
